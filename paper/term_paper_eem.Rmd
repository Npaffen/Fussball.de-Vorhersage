...
stargazer(fake,fake,fake,
          ...
          t.auto = T,
          dep.var.caption  = "Dependet variable = goals",
...
stargaz <- readLines(str_c(here::here(), "/paper/stargazer_data.html"))
stargaz <- stargaz[-5]
writeLines(stargaz,str_c(here::here(), "/paper/stargazer_data.html"))
```
...
average_table_poisson <- average_table %>% arrange(rank) %>% select(rank, everything(), -run)%>% rename(club_name_poisson = club_name, score_poisson = score)
kable(inner_join(average_table_poisson,average_table_elo, by = "rank"),
      caption = "Simulated Final Score Table"
                    ) %>%
  add_header_above(c(" " = 1, "Poisson Distribution" = 2, "Elo Rating" = 2 )) %>%
