...
```\{r , results = \enquote{asis}\}\#Does not work right now
library(tidyverse) require(stargazer) library(knitr)
opts\_chunk\$set(echo = TRUE)

database\_mr \textless{}- readr::read\_rds(stringr::str\_c(here::here()
, \enquote{data}, \enquote{database\_match\_results\_1920.rds}, sep =
\enquote{/}))

home\_goals\_avg \textless{}- database\_mr\$goals\_team\_home
\%\textgreater{}\% mean()

away\_goals\_avg \textless{}- database\_mr\$goals\_team\_away
\%\textgreater{}\% mean()

poisson\_model \textless{}- bind\_rows( tibble( goals =
database\_mr\(goals_team_home,  team = database_mr\)club\_name\_home,
opponent=database\_mr\(club_name_away,  home=1),  tibble(  goals=database_mr\)goals\_team\_away,
team=database\_mr\(club_name_away,  opponent=database_mr\)club\_name\_home,
home=0)) \%\textgreater{}\%

glm(goals \textasciitilde{} home + team +opponent,
family=poisson(link=log),data=.) half \textless{}- list(coefficients
=list(poisson\_model\$coefficients{[}1:17{]}))

stargazer(half, title = \enquote{These are awesome results!},
notes=\enquote{stargazer html}, no.space = T) ```
